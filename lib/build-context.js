 ...
 */
function formatContext(profile, experience, projects, skills, certifications, faq) {
  let context = '';
  ...
  if (certifications && certifications.length > 0) {
    certifications.forEach(cert => {
      context += `- Name: ${cert.name}\n`;
      if (cert.url) context += `  Verification Link: ${cert.url}\n`;
    });
    context += `\n`;
  } else {
    context += `(No certifications recorded)\n\n`;
  }
